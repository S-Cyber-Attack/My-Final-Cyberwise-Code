...
# Alink to see the information given by the external IP
def ip_tracker_link():
    d = str(urlopen('http://checkip.dyndns.com/').read())
...
def network_scanner_with_logs():
    ...
    class Device(object):
        ...
        def to_string(self):
            ...
            return 'Log: {} \n\t Mac Address: {} \n\t Name in network: {} \n\t Given name: {} \n\t Allowed on network: {}'.format(
                datetime.now(), self.mac, self.network_name, self.name, self.allowed)
...
def all_past_wifi():
    ...
    for i in data:
        ...
        # as this item will have the wi-fi name
...
# States if open ports are filtered
...
def repeat():
    # This can be replaced by any subroutine to indefinitely repeat
    def task():
        print('Running an endless task now... (Pause for 5 seconds)')
